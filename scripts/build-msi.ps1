...
#  Gera o executável standalone (pkg) e o instalador .msi (WiX v3)
...
Write-Host "[1/5] Preparando diretórios..." -ForegroundColor Yellow
New-Item -ItemType Directory -Force -Path $DistDir | Out-Null
New-Item -ItemType Directory -Force -Path (Join-Path $ProjectRoot "tools") | Out-Null
Write-Host "      ✔ dist/ e tools/ prontos" -ForegroundColor Green

# ─── Step 2: Build standalone .exe with pkg ──────────────────────────────────
Write-Host "[2/5] Empacotando com pkg (Node.js standalone .exe)..." -ForegroundColor Yellow
Write-Host "      Isso pode levar 1-3 minutos na primeira vez..." -ForegroundColor Gray

Set-Location $ProjectRoot
& npx pkg . --targets node18-win-x64 --output $ExePath --compress GZip

if ($LASTEXITCODE -ne 0) {
    Write-Host "  ✘ Falha ao empacotar com pkg. Verifique os erros acima." -ForegroundColor Red
    exit 1
}

$ExeMB = [math]::Round((Get-Item $ExePath).Length / 1MB, 1)
Write-Host "      ✔ $ExeName ($ExeMB MB) criado em dist\" -ForegroundColor Green
...
if (-not (Test-Path $CandleExe)) {
    Write-Host "      Baixando WiX v3 binários (~8 MB)..." -ForegroundColor Gray
    ...
    Write-Host "      ✔ WiX extraído em tools\wix\" -ForegroundColor Green
} else {
    Write-Host "      ✔ WiX já disponível em tools\wix\" -ForegroundColor Green
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "  ✘ Falha na compilação WiX (candle.exe). Verifique o .wxs." -ForegroundColor Red
    exit 1
}
Write-Host "      ✔ .wixobj gerado" -ForegroundColor Green
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "  ✘ Falha ao criar o .msi (light.exe)." -ForegroundColor Red
    exit 1
}
...
Write-Host "  ╚══════════════════════════════════════════════════════════╝" -ForegroundColor Green
Write-Host ""
Write-Host "  📦 Executável  : dist\$ExeName  ($ExeMB MB)" -ForegroundColor White
...
Write-Host "  2. Faça upload do .msi no GitHub Release v1.0.0:" -ForegroundColor Cyan
